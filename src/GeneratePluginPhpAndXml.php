...
final class GeneratePluginPhpAndXml
{
    ...
    public static function generatePluginPhpAndXml(Event $event)/*: void*/
    {
        ...
        self::getInstance($plugin_root)->doGeneratePluginPhpAndXml();
    }
     ...
     *
     */
    public function doGeneratePluginPhpAndXml()/*: void*/
    {
        $this->plugin_composer_json = json_decode(file_get_contents($this->plugin_root . "/" . self::PLUGIN_COMPOSER_JSON));
        ...
        $this->plugin_composer_json = null;
    }
...
";
            ...
            file_put_contents($this->plugin_root . "/" . self::LUCENE_OBJECT_DEFINITION_XML, '<?xml version="1.0" encoding="UTF-8"?>
...
";
        ...
        file_put_contents($this->plugin_root . "/" . self::PLUGIN_PHP, '<?php
...
";
        ...
        file_put_contents($this->plugin_root . "/" . self::PLUGIN_XML, '<?php xml version = "1.0" encoding = "UTF-8"?>
<!-- ' . htmlspecialchars(self::AUTOGENERATED_COMMENT) . ' -->
...
');
    ...
    {
        if (file_exists($this->plugin_root . "/" . self::PLUGIN_PHP)) {
            $plugin_php = file_get_contents($this->plugin_root . "/" . self::PLUGIN_PHP);
...
";
        ...
        $name = basename($this->plugin_root);
...
";
        ...
        if (empty($this->plugin_composer_json->extra->ilias_plugin->slot)) {
            ...
            $plugin_class_code = file_get_contents($this->plugin_root . "/" . $plugin_class);
            ...
            $matches = [];
            $readme = file_get_contents($this->plugin_root . "/" . self::PLUGIN_README);
...
";
        ...
        if (empty($this->plugin_composer_json->extra->ilias_plugin->lucene_search)) {
            $lucene_search = json_encode(file_exists($this->plugin_root . "/" . self::LUCENE_OBJECT_DEFINITION_XML));
...
";
        ...
        if (empty($this->plugin_composer_json->extra->ilias_plugin->events)) {
            if (file_exists($this->plugin_root . "/" . self::PLUGIN_XML)) {
                $plugin_xml = json_decode(json_encode(simpleXML_load_file($this->plugin_root . "/" . self::PLUGIN_XML)));
...
";
            ...
            file_put_contents($this->plugin_root . "/" . self::PLUGIN_COMPOSER_JSON, preg_replace_callback("/\n( +)/", function (array $matches) : string {
                    return "
