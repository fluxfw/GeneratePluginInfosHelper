...
final class GeneratePluginPhpAndXml
{
    ...
    public static function generatePluginPhpAndXml(Event $event)/*: void*/
    {
        ...
        self::getInstance()->doGeneratePluginPhpAndXml($plugin_root, true);
    }
     ...
     * @param bool   $log
     */
    public function doGeneratePluginPhpAndXml(string $plugin_root, bool $log = false)/*: void*/
    {
        $plugin_composer_json = json_decode(file_get_contents($plugin_root . "/" . self::PLUGIN_COMPOSER_JSON));

        $this->updateMissingVariablesComposerJson($plugin_composer_json, $plugin_root, $log);

        $this->generatePluginPhp($plugin_composer_json, $plugin_root, $log);

        $this->generatePluginXml($plugin_composer_json, $plugin_root, $log);

        $this->generateLuceneObjectDefinitionXml($plugin_composer_json, $plugin_root, $log);
    }
     ...
     * @param bool     $log
     */
    private function generateLuceneObjectDefinitionXml(stdClass $plugin_composer_json, string $plugin_root, bool $log)/* : void*/
    {
...
";
            ...
            file_put_contents($plugin_root . "/" . self::LUCENE_OBJECT_DEFINITION_XML, '<?xml version="1.0" encoding="UTF-8"?>
...
');
     ...
     * @param bool     $log
     */
    private function generatePluginPhp(stdClass $plugin_composer_json, string $plugin_root, bool $log)/* : void*/
    {
        if ($log) {
...
";
        ...
        file_put_contents($plugin_root . "/" . self::PLUGIN_PHP, '<?php
...
');
     ...
     * @param bool     $log
     */
    private function generatePluginXml(stdClass $plugin_composer_json, string $plugin_root, bool $log)/* : void*/
    {
        if ($log) {
...
";
        ...
        file_put_contents($plugin_root . "/" . self::PLUGIN_XML, '<?php xml version = "1.0" encoding = "UTF-8"?>
<!-- ' . htmlspecialchars(self::AUTOGENERATED_COMMENT) . ' -->
...
');
     ...
     * @return string
     */
    private function getOldPluginVar(string $variable, string $plugin_root) : string
    {
        if (file_exists($plugin_root . "/" . self::PLUGIN_PHP)) {
            $plugin_php = file_get_contents($plugin_root . "/" . self::PLUGIN_PHP);
     ...
     * @param bool     $log
     */
    private function updateMissingVariablesComposerJson(stdClass $plugin_composer_json, string $plugin_root, bool $log)/* : void*/
    {
        $updated_composer_json = false;

        $old_version = $this->getOldPluginVar("version", $plugin_root);
...
";
        ...
        $id = $this->getOldPluginVar("id", $plugin_root);
...
";
        ...
        $old_ilias_min_version = $this->getOldPluginVar("ilias_min_version", $plugin_root);
...
";
        ...
        $old_ilias_max_version = $this->getOldPluginVar("ilias_max_version", $plugin_root);
...
";
        ...
        if (empty($plugin_composer_json->extra->ilias_plugin->slot)) {
            ...
            $plugin_class_code = file_get_contents($plugin_root . "/" . $plugin_class);
            ...
            $matches = [];
            $readme = file_get_contents($plugin_root . "/" . self::PLUGIN_README);
...
";
        ...
        if (empty($plugin_composer_json->extra->ilias_plugin->learning_progress)) {
            $learning_progress = $this->getOldPluginVar("learning_progress", $plugin_root);
...
";
        ...
        if (empty($plugin_composer_json->extra->ilias_plugin->lucene_search)) {
            $lucene_search = json_encode(file_exists($plugin_root . "/" . self::LUCENE_OBJECT_DEFINITION_XML));
...
";
        ...
        if (empty($plugin_composer_json->extra->ilias_plugin->supports_export)) {
            $supports_export = $this->getOldPluginVar("supports_export", $plugin_root);
...
";
        ...
        if (empty($plugin_composer_json->authors)) {
            $responsible = $this->getOldPluginVar("responsible", $plugin_root);
            $responsible_mail = $this->getOldPluginVar("responsible_mail", $plugin_root);
...
";
        ...
        if (empty($plugin_composer_json->extra->ilias_plugin->events)) {
            if (file_exists($plugin_root . "/" . self::PLUGIN_XML)) {
                $plugin_xml = json_decode(json_encode(simpleXML_load_file($plugin_root . "/" . self::PLUGIN_XML)));
...
";
            ...
            file_put_contents($plugin_root . "/" . self::PLUGIN_COMPOSER_JSON, preg_replace_callback("/\n( +)/", function (array $matches) : string {
                    return "
