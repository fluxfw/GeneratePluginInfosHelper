...
";
        ...
        $authors = $plugin_composer_json->authors;
        if (empty($authors)) {
            $authors = [];
        }
        $author = $authors[0];
        ...
        $require = $plugin_composer_json->require;
        if (empty($require)) {
            $require = (object) [];
        }

        $support = $plugin_composer_json->support;
        if (empty($support)) {
            $support = (object) [];
        }

        $placeholders = [
            "AUTHOR_EMAIL"                   => strval($author->email ?? ""),
            "AUTHOR_HOMEPAGE"                => strval($author->homepage ?? ""),
            "AUTHOR_NAME"                    => strval($author->name ?? ""),
            ...
            "PHP_VERSION"                    => strval($require->php ?? ""),
            "SHORT_DESCRIPTION"              => strval($plugin_composer_json->description ?? ""),
            "SUPPORT_LINK"                   => strval($support->issues ?? ""),
